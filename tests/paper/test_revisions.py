...
class DescribeUnresolvableRevisions:
    ...
    def it_reports_a_census_of_unsupported_revisions(self):
        ...
        document = _doc(self.MOVES)
        tbl = document.add_table(rows=1, cols=1)._tbl
        from docx.oxml import parse_xml
        from docx.oxml.ns import nsdecls

        tbl.tblPr.append(
            parse_xml(
                f'<w:tblPrChange {nsdecls("w")} w:id="991" w:author="A"'
                ' w:date="2026-06-01T09:30:00Z"><w:tblPr/></w:tblPrChange>'
            )
        )
        assert document.revisions.remaining_unsupported() == {
            "table_property_change": 1
        }

    def it_refuses_individual_resolution_of_an_exotic_revision(self):
        document = _doc(self.MOVES)
        tbl = document.add_table(rows=1, cols=1)._tbl
        from docx.oxml import parse_xml
        from docx.oxml.ns import nsdecls
        ...
        tbl.tblPr.append(
            parse_xml(
                f'<w:tblPrChange {nsdecls("w")} w:id="992" w:author="A"'
                ' w:date="2026-06-01T09:30:00Z"><w:tblPr/></w:tblPrChange>'
            )
        )
        revision = next(
            ...
            if r.revision_type == "table_property_change"
        )
    ...
    def it_resolves_an_author_filtered_clean_subset_alongside_moves(self, tmp_path):
        """Selected-set semantics: Carol's plain insertion resolves while
        Alice's move stays pending (resolvable, so absent from the census)."""
        import shutil
